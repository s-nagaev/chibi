"""Unit tests for the DeepInfra provider adapter."""
...
from chibi.services.providers import DeepInfra, RegisteredProviders

TEST_TOKEN = "test-deepinfra-token"
...
def _make_provider() -> DeepInfra:
    return DeepInfra(token=TEST_TOKEN)
...
def test_deepinfra_registered_in_registered_providers() -> None:
    """The class is registered under its lowercase provider name on import."""
    assert RegisteredProviders.all["deepinfra"] is DeepInfra


def test_availability_gated_on_api_key() -> None:
    """The provider is only marked available when a truthy api_key is configured."""
    assert DeepInfra.api_key == gpt_settings.deepinfra_key
    if DeepInfra.api_key:
        assert "deepinfra" in RegisteredProviders.available
    else:
        assert "deepinfra" not in RegisteredProviders.available
...
def test_model_filtering_chat_ready_models_sorted() -> None:
    ...
    models = [
        ModelChangeSchema(
            provider="Deepinfra", name="meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo", image_generation=False
        ),
        ModelChangeSchema(provider="Deepinfra", name="openai/gpt-oss-120b", image_generation=False),
    ]
...
def test_model_filtering_image_generation() -> None:
    ...
    provider = _make_provider()
    models = [
        ModelChangeSchema(provider="Deepinfra", name="black-forest-labs/FLUX-1-schnell", image_generation=True),
        ModelChangeSchema(provider="Deepinfra", name="deepseek-ai/DeepSeek-V4-Flash-0731", image_generation=False),
    ]
