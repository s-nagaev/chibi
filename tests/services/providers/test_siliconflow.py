"""Unit tests for the SiliconFlow provider adapter."""
...
from chibi.services.providers import RegisteredProviders, SiliconFlow

TEST_TOKEN = "test-siliconflow-token"
...
def _make_openai_error(error_cls: type[OpenAIError], status_code: int, code: str | None) -> OpenAIError:
    ...
    request = httpx.Request("POST", "https://api.siliconflow.com/v1/chat/completions")
...
def _make_provider() -> SiliconFlow:
    return SiliconFlow(token=TEST_TOKEN)
...
def test_siliconflow_registered_in_registered_providers() -> None:
    """The class is registered under its lowercase provider name on import."""
    assert RegisteredProviders.all["siliconflow"] is SiliconFlow


def test_availability_gated_on_api_key() -> None:
    """The provider is only marked available when a truthy api_key is configured."""
    assert SiliconFlow.api_key == gpt_settings.siliconflow_key
    if SiliconFlow.api_key:
        assert "siliconflow" in RegisteredProviders.available
    else:
        assert "siliconflow" not in RegisteredProviders.available
...
def test_model_filtering_chat_ready_models_sorted() -> None:
    ...
    models = [
        ModelChangeSchema(provider="Siliconflow", name="deepseek-ai/DeepSeek-V4-Pro", image_generation=False),
        ModelChangeSchema(provider="Siliconflow", name="zai-org/GLM-5", image_generation=False),
    ]
...
def test_model_filtering_image_generation() -> None:
    ...
    provider = _make_provider()
    models = [
        ModelChangeSchema(provider="Siliconflow", name="black-forest-labs/FLUX.1-schnell", image_generation=True),
        ModelChangeSchema(provider="Siliconflow", name="deepseek-ai/DeepSeek-V4-Flash", image_generation=False),
    ]
