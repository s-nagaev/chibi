"""Unit tests for the Baseten provider adapter."""
...
from chibi.services.providers import Baseten, RegisteredProviders

TEST_TOKEN = "test-baseten-token"
...
def _make_provider() -> Baseten:
    return Baseten(token=TEST_TOKEN)
...
def test_baseten_registered_in_registered_providers() -> None:
    """The class is registered under its lowercase provider name on import."""
    assert RegisteredProviders.all["baseten"] is Baseten


def test_availability_gated_on_api_key() -> None:
    """The provider is only marked available when a truthy api_key is configured."""
    assert Baseten.api_key == gpt_settings.baseten_key
    if Baseten.api_key:
        assert "baseten" in RegisteredProviders.available
    else:
        assert "baseten" not in RegisteredProviders.available
...
def test_model_filtering_chat_ready_models_sorted() -> None:
    ...
    models = [
        ModelChangeSchema(provider="Baseten", name="zai-org/GLM-5.3-Flash", image_generation=False),
        ModelChangeSchema(provider="Baseten", name="openai/gpt-oss-120b", image_generation=False),
    ]
...
def test_model_filtering_image_generation() -> None:
    ...
    provider = _make_provider()
    models = [
        ModelChangeSchema(provider="Baseten", name="black-forest-labs/FLUX.1-schnell", image_generation=True),
        ModelChangeSchema(provider="Baseten", name="openai/gpt-oss-120b", image_generation=False),
    ]
