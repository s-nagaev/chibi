"""Unit tests for the Nebius provider adapter."""
...
from chibi.services.providers import Nebius, RegisteredProviders

TEST_TOKEN = "test-nebius-token"
...
def _make_provider() -> Nebius:
    return Nebius(token=TEST_TOKEN)
...
def test_nebius_registered_in_registered_providers() -> None:
    """The class is registered under its lowercase provider name on import."""
    assert RegisteredProviders.all["nebius"] is Nebius


def test_availability_gated_on_api_key() -> None:
    """The provider is only marked available when a truthy api_key is configured."""
    assert Nebius.api_key == gpt_settings.nebius_key
    if Nebius.api_key:
        assert "nebius" in RegisteredProviders.available
    else:
        assert "nebius" not in RegisteredProviders.available
...
def test_model_filtering_image_generation() -> None:
    ...
    models = [
        ...
        ModelChangeSchema(
            provider="Nebius",
            name="nvidia/Nemotron-3_5-Lightning",
            image_generation=False,
        ),
    ]
