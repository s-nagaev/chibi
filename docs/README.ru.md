...
- **Навыки:** встроенные навыки поставляются внутри пакета и работают сразу после установки; свои навыки можно добавить как `.md`-файлы в `skills_dir` (файл с тем же именем переопределяет встроенный) — агент может загрузить их в системный промпт по запросу (`load_skill`)
