...
**創意工具：**
- **ElevenLabs** (語音): [elevenlabs.io](https://elevenlabs.io/)
- **Suno** (音樂，非官方): [sunoapi.org](https://sunoapi.org/)
...
- **技能**：內建技能隨套件一同發布，安裝後即可使用；將自訂的 `.md` 檔案放入 `skills_dir` 即可新增技能（同名檔案會覆蓋內建技能）；代理可按需將其載入到系統提示中（`load_skill`）
