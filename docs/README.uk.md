# Chibi - Ваш AI-компаньйон

**Chibi** - це не просто ще один чат-бот. Це **оркестратор**, **агент** і **компаньйон**, що живе у вашому Telegram.

Він створений для **реальної роботи**: керування серверами, написання коду, дослідження тем та керування іншими AI-моделями. Chibi об’єднує найкращі LLM (OpenAI, Anthropic, Google, DeepSeek тощо) в єдиний, цілісний інтерфейс.

### Чому Chibi?

- **Незалежність від провайдера.** Використовуйте найкращу модель для кожної задачі - без прив'язки до вендора (vendor lock-in).
- **Автономне виконання.** Субагенти працюють паралельно; довгі задачі виконуються асинхронно.
...
- **Зображення:** Google (Imagen, Nano Banana), OpenAI (DALL·E), Alibaba (Qwen Image), xAI (Grok Image), Wan
- **Музика:** Suno
...
Бот працюватиме у фоні. Використовуйте CLI-команди для керування.
